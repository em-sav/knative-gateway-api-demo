...
kubectl apply -f "$REPO_DIR/k8s/envoy-gateway/config.yaml"
...
# kubectl wait --timeout=5m -n metallb-system daemonset/metallb-speaker --for=condition=Available
kubectl apply -f "$REPO_DIR/k8s/metallb/config.yaml"

# Install and Configure Knative Gateway
kubectl apply -f "$REPO_DIR/k8s/gateway/config.yaml"
